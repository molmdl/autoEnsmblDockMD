...
1. CLI mode:
     python scripts/dock/0_gro_itp_to_mol2.py --gro in.gro --itp in.itp --output out.mol2
2. Library mode:
...
DEFAULT_CONFIG_SECTION = "dock"

LOGGER = logging.getLogger("gro_itp_to_mol2")
...
CLI_FLAG_GRO = "--gro"
CLI_FLAG_ITP = "--itp"
CLI_FLAG_OUT = "--out"  # backward-compatible alias
CLI_FLAG_OUTPUT = "--output"
CLI_FLAG_NAME = "--name"
...
CLI_FLAG_CONFIG = "--config"
CLI_FLAG_LOG_LEVEL = "--log-level"
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(CLI_FLAG_GRO, type=Path, help="Input GRO file path")
    parser.add_argument(CLI_FLAG_ITP, type=Path, help="Input ITP file path")
    parser.add_argument(CLI_FLAG_OUTPUT, type=Path, help="Output MOL2 file path")
    parser.add_argument(CLI_FLAG_OUT, dest="out", type=Path, help=argparse.SUPPRESS)
    parser.add_argument(
        CLI_FLAG_CONFIG,
        type=Path,
        help="Optional INI config file ([dock] section: gro, itp, output, ...)",
    )
    parser.add_argument(
        CLI_FLAG_NAME,
        default="",
    ...
    if args.output is None and args.out is not None:
...
def validate_input_files(gro_path: Path, itp_path: Path) -> None:
    if not gro_path.exists() or not gro_path.is_file():
        raise FileNotFoundError(f"GRO file not found: {gro_path}")
    if not itp_path.exists() or not itp_path.is_file():
        raise FileNotFoundError(f"ITP file not found: {itp_path}")
    if gro_path.suffix.lower() != ".gro":
        raise ValueError(f"Input GRO path does not look like a .gro file: {gro_path}")
    # Allow .top for historical compatibility because some ligand topologies are named *_gmx.top.
    if itp_path.suffix.lower() not in {".itp", ".top"}:
        raise ValueError(f"Input ITP path must end with .itp or .top: {itp_path}")


def load_config_overrides(config_path: Path) -> Dict[str, str]:
    if not config_path.exists() or not config_path.is_file():
        raise FileNotFoundError(f"Config file not found: {config_path}")

    parser = configparser.ConfigParser()
    parser.read(config_path)
    if not parser.has_section(DEFAULT_CONFIG_SECTION):
        return {}
    return {k: v for k, v in parser.items(DEFAULT_CONFIG_SECTION)}
...
def convert(
    ...
    *,
    name: str = "",
    keep_itp_types: bool = DEFAULT_USE_ITP_TYPES_DIRECTLY,
    no_aromatic_bonds: bool = not DEFAULT_ENABLE_AROMATIC_BONDS,
    no_unique_atom_names: bool = not DEFAULT_ENABLE_UNIQUE_ATOM_NAMES,
) -> Path:
    gro_path = Path(gro)
    itp_path = Path(itp)
    output_path = Path(output) if output is not None else None

    validate_input_files(gro_path, itp_path)
    resolved_output = resolve_output_path(gro_path=gro_path, output_path=output_path)

    LOGGER.info("Reading GRO: %s", gro_path)
    gro_title, gro_atoms = parse_gro_file(path=gro_path, coordinate_scale=DEFAULT_COORDINATE_SCALE)
    LOGGER.info("Reading ITP: %s", itp_path)
    ...
    molecule_name = name or itp_name or gro_title or DEFAULT_FALLBACK_MOLECULE_NAME
    enable_aromatic_bonds = DEFAULT_ENABLE_AROMATIC_BONDS and (not no_aromatic_bonds)
    use_unique_names = DEFAULT_ENABLE_UNIQUE_ATOM_NAMES and (not no_unique_atom_names)
    ...
    LOGGER.info("Writing MOL2: %s", resolved_output)
    write_mol2_file(
    ...
    LOGGER.info(
        "Done. Atoms=%d Bonds=%d Substructures=%d",
        len(mol2_atoms),
        len(mol2_bonds),
        len(mol2_substructures),
    )
    return resolved_output


def _resolve_cli_values(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.gro is None and "gro" in overrides:
        args.gro = Path(overrides["gro"])
    if args.itp is None and "itp" in overrides:
        args.itp = Path(overrides["itp"])
    if args.output is None and "output" in overrides:
        args.output = Path(overrides["output"])
    if not args.name and "name" in overrides:
        args.name = overrides["name"]

    # Boolean overrides honor CLI flags first.
    if "keep_itp_types" in overrides and not args.keep_itp_types:
        args.keep_itp_types = overrides["keep_itp_types"].strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        }
    if "no_aromatic_bonds" in overrides and not args.no_aromatic_bonds:
        args.no_aromatic_bonds = overrides["no_aromatic_bonds"].strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        }
    if "no_unique_atom_names" in overrides and not args.no_unique_atom_names:
        args.no_unique_atom_names = overrides["no_unique_atom_names"].strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
...
def main() -> None:
    ...
    args = _resolve_cli_values(args)

    if args.gro is None or args.itp is None:
        raise SystemExit("Error: --gro and --itp are required (directly or via --config [dock]).")
    ...
    output_path = convert(
        ...
        no_aromatic_bonds=args.no_aromatic_bonds,
        no_unique_atom_names=args.no_unique_atom_names,
    )
