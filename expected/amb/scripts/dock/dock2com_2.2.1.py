...
New Features in dock2com_2.2.1.py:
- Fixed manual model selection (`--model`) so it no longer gets overridden by
...
-------------
    # Basic usage
    python dock2com_2.2.1.py -i lig.itp -s hsa*-phe_sssD.sdf -r topol.top -t lig.mol2

    # Select by SASA (lower = more buried = better)
    python dock2com_2.2.1.py -i lig.itp -s hsa*-phe_sssD.sdf -r topol.top -t lig.mol2 --metric sasa

    # List all models with scores
    python dock2com_2.2.1.py -i lig.itp -s hsa*-phe_sssD.sdf --list-models
    
    # With explicit ffbonded.itp (CHARMM CGenFF)
    python dock2com_2.2.1.py -i lig.itp -s rec-lig.sdf -r topol.top --lig-ffbonded lig_ffbonded.itp
"""
...
def main():
    ...
    if config.model_num is not None:
        print("STEP 1: Selecting docking model (manual override)...")
    else:
    ...
    if config.model_num is not None:
        if len(config.sdf_paths) != 1:
            parser.error("--model requires exactly one SDF file.")
        
        best_file = config.sdf_paths[0]
        print(f"Manual model requested: {config.model_num}")
        print(f"Parsing SDF:        {best_file}")
        models = parse_sdf(best_file)
        print(f"  {len(models)} models found")
        
        if not models:
            parser.error(f"No models found in SDF file: {best_file}")
        if config.model_num < 1 or config.model_num > len(models):
            parser.error(
                f"--model {config.model_num} is out of range for "
                f"{os.path.basename(best_file)} (valid range: 1-{len(models)})."
            )
        
        best_model_num = config.model_num
        best_model = models[best_model_num - 1]
    else:
