...
-------------
    ...
    python dock2com_2.1.py -i lig.itp -s hsa*-phe_sssD.sdf -r topol.top -t lig.mol2

    # Select by SASA (lower = more buried = better)
    python dock2com_2.1.py -i lig.itp -s hsa*-phe_sssD.sdf -r topol.top -t lig.mol2 --metric sasa

    # List all models with scores
    python dock2com_2.1.py -i lig.itp -s hsa*-phe_sssD.sdf --list-models
"""
...
# Halogen atom types for CGenFF and other force fields
# Format: atom_type_prefix -> element
HALOGEN_ATOM_TYPES = {
    'br': 'Br', 'BR': 'Br',
    'cl': 'Cl', 'CL': 'Cl',
    'fl': 'F', 'FL': 'F',
...
def _element(atom_type):
    """Extract element from mol2 atom type, with halogen support."""
...
def _itp_element(atom_type):
    """
    Extract element from ITP atom type, with halogen support.
    
    Handles CGenFF halogen types like:
    - BRGR1, BRGR2 (Bromine)
    - CLGR1, CLGR2 (Chlorine)
    - FGR1, FGR2 (Fluorine)
    - IGR1, IGR2 (Iodine)
    """
...
def main():
    ...
    print("\nDock2com_2.1 completed successfully!")
