...
Extended with SASA-based selection metric.
...
-------------
    ...
    python dock2com_2.py -i lig.itp -s hsa*-phe_sssD.sdf -r topol.top -t lig.mol2 --metric sasa
...
SASA_GAUSSIAN_LAMBDA = 0.5  # Scaling factor for burial
SASA_GAUSSIAN_SIGMA = 1.5   # Gaussian width in Angstroms
SASA_NEIGHBOR_CUTOFF = 10.0 # Neighbor search cutoff in Angstroms

# VDW radii table (Bondi 1964 + Mantina 2009 extensions)
# Copied from quick_sasa.py for complete coverage
VDW_RADII_ANG = {
...
VDW_DEFAULT_RADIUS = 1.70  # Carbon fallback

# ---------------------------------------------------------------------------
# Element Inference for GRO Parsing
# ---------------------------------------------------------------------------
...
def infer_element_from_atom_name(atom_name):
    ...
    GRO files do not store element information directly. This function
    infers the element from standard PDB/GROMACS atom naming conventions.
    
    Parameters
    ----------
    atom_name : str
        Atom name from GRO file (e.g., "CA", "NH1", "HZ", "EU")
    
    Returns
    -------
    str
        Element symbol (e.g., "C", "N", "H", "Eu")
    
    Examples
    --------
    >>> infer_element_from_atom_name("CA")
    'C'
    >>> infer_element_from_atom_name("NH1")
    'N'
    >>> infer_element_from_atom_name("EU")
    'Eu'
    >>> infer_element_from_atom_name("FE")
    'Fe'
    """
    # Special case mappings for common atom names
    special_map = {
        # Carbon atoms with suffixes
        "CA": "C",   # Alpha carbon
        "CB": "C",   # Beta carbon
        "CD": "C",   # Delta carbon
        "CD1": "C", "CD2": "C",
        ...
        "CZ2": "C", "CZ3": "C",
        "CH2": "C",
        # Nitrogen atoms with suffixes
        "NH": "N", "NH1": "N", "NH2": "N",
        "ND1": "N", "ND2": "N",
        "NE": "N", "NE1": "N", "NE2": "N",
        "NZ": "N",
        # Oxygen atoms with suffixes
        "OH": "O",   # Hydroxyl oxygen
        "OD": "O", "OD1": "O", "OD2": "O",
        "OE": "O", "OE1": "O", "OE2": "O",
        "OG": "O", "OG1": "O", "OG2": "O",
        "OXT": "O",  # Terminal oxygen
        # Sulfur atoms
        "SD": "S",   # Sulfur delta
        "SG": "S",   # Sulfur gamma
        # Metals (preserve proper capitalization)
        "EU": "Eu",  # Europium
        "FE": "Fe",  # Iron
        "ZN": "Zn",  # Zinc
        "MG": "Mg",  # Magnesium
        "CA": "C",   # Note: CA in proteins is carbon, not calcium
        "MN": "Mn",  # Manganese
        "CU": "Cu",  # Copper
        "NI": "Ni",  # Nickel
        "CO": "Co",  # Cobalt
    ...
    # Handle numbered hydrogens: H1, H2, H3, HA, HB, etc.
    if upper.startswith('H'):
        return "H"
    
    # Two-character elements that might be in VDW table
    if len(upper) >= 2:
        two_char = upper[:2]
        # Check if it's a known element (with proper capitalization)
        if two_char.capitalize() in VDW_RADII_ANG:
            return two_char.capitalize()
        if two_char.upper() in VDW_RADII_ANG:
            return two_char.upper()
        # Check if first character + second digit (e.g., C1, N1, O1)
        if two_char[1].isdigit():
            one_char = two_char[0]
            if one_char in VDW_RADII_ANG:
                return one_char
    
    # Fall back to first character (most common case)
    if upper and upper[0] in VDW_RADII_ANG:
        return upper[0]
    
    # Ultimate fallback: return first character as-is
...
def calculate_pose_sasa(ligand_atoms, receptor_atoms, probe_radius=1.4):
    """
    Calculate total ligand SASA in context of receptor using VDW+Gaussian approximation.
    
    This is a fast approximation method suitable for relative comparisons (ranking
    poses by burial). Uses the same algorithm as quick_sasa.py.
    
    Algorithm
    ---------
    For each ligand atom i:
        SASA_i = 4 * pi * (r_vdw + r_probe)^2 * exp(-lambda * neighbor_sum)
    
    where neighbor_sum is a Gaussian-weighted count of nearby atoms.
    ...
    receptor_atoms : list of dict
        Same format as ligand_atoms (coordinates in Angstroms)
    probe_radius : float
    ...
    -------
    float
        Total ligand SASA in nm² (lower = more buried = better)
    
    Notes
    -----
    - Speed: ~0.7ms per pose for typical protein-ligand complexes
    - Accuracy: 10-20% vs. probe-based methods (suitable for relative ranking)
    - Output in nm² for consistency with GROMACS conventions
    """
    if not ligand_atoms:
        return 0.0
    
    # Combine all coordinates for neighbor search
    all_coords = np.array(
    ...
    try:
        ...
        # Fallback: simple loop (slower but works without scipy)
        tree = None
    ...
    for i, atom in enumerate(ligand_atoms):
        ...
        # Get VDW radius for this element
...
def parse_gro_for_sasa(gro_path):
    ...
    list of dict
        ...
        Element inferred from atom name
    
    Raises
    ------
    FileNotFoundError
        If gro_path does not exist
    """
...
def select_best_across_sdfiles(
    ...
    """
    Select best model across SDF files, with SASA support.
    
    Extended from dock2com_1.py to support SASA as a selection metric.
    When metric == "sasa", calculates SASA for each pose and selects
    the one with lowest SASA (most buried).
    ...
    metric : str
        Selection metric. Options: "minimizedAffinity", "CNNscore", 
        "CNNaffinity", "sasa" (new)
    rec_gro_pattern : str, optional
        Pattern for auto-detecting receptor GRO files
    rec_dir : str, optional
        Directory containing receptor files
    probe_radius : float
        Solvent probe radius for SASA calculation (default: 1.4)
    
    Returns
    -------
    best_model : dict
        Selected model with scores (including SASA if calculated)
    best_file : str
    ...
    Notes
    -----
    - Receptor GRO is loaded ONCE per SDF file (not per model) for efficiency
    - SASA is only calculated when metric == "sasa"
    - Graceful degradation: if receptor not found, falls back to affinity metric
    """
    best_model = None
    best_score = None
    best_file = None
    ...
    # Track if we fell back due to missing receptor
    ...
    for sdf_path in sdf_paths:
        ...
        # Load receptor ONCE per SDF file (optimization from verification.md)
        receptor_atoms = None
        rec_gro = None
        ...
        # If SASA requested but no receptor available, fall back to affinity
...
def list_models_across_sdfiles(sdf_paths, metrics=None, rec_gro_pattern=None, rec_dir=None, probe_radius=DEFAULT_PROBE_RADIUS):
    ...
    for sdf_path in sdf_paths:
        ...
        # Load receptor for SASA calculation if "sasa" in metrics
        receptor_atoms = None
        if "sasa" in metrics:
            if rec_gro_pattern is None:
                rec_gro_pattern = DEFAULT_REC_GRO_PATTERN
...
def main():
    ...
    print("\nDock2com_2 completed successfully!")
